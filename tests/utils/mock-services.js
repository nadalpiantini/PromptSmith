...
export function createMockFileSystem() {
    ...
    return {
        ...
        exists: jest.fn().mockImplementation((path) => Promise.resolve(files.has(path))),
...
export function createMockWebhookService() {
    const deliveries = [];
    return {
        send: jest.fn().mockImplementation((url, payload) => {
...
export function createMockQueueService() {
    ...
    return {
        ...
        process: jest.fn().mockImplementation((processor) => {
            // Mock job processing
            setImmediate(() => {
...
export function createMockAnalyticsService() {
    const events = [];
    return {
        track: jest.fn().mockImplementation((event, properties) => {
            events.push({
                event,
                properties,
                timestamp: new Date(),
        ...
        identify: jest.fn().mockImplementation((userId, traits) => {
            events.push({
                event: 'identify',
